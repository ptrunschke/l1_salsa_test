...
if __name__ == "__main__":
    ...
    model_skl = LassoLarsCV(normalize=False, fit_intercept=False, cv=10).fit(
        np.nan_to_num(measures / basisWeights[np.newaxis]), values
    )
    ...
    model_skl_normalized = LassoLarsCV(normalize=True, fit_intercept=False, cv=10).fit(
        np.nan_to_num(measures / basisWeights[np.newaxis]), values
    )
    ...
    print(
        f"Coefficient range: [{np.min(abs(own_model.coef_ / basisWeights[own_model.active_])):.2e}, {np.max(abs(own_model.coef_ / basisWeights[own_model.active_])):.2e}]"
    )
    coefs = np.zeros(d)
    ...
    print(f"Missing indices: {skla - lsa}")
    print(
        f"Coefficient range: [{np.min(abs(own_model_norm.coef_ / infty_norms[own_model_norm.active_])):.2e}, {np.max(abs(own_model_norm.coef_ / infty_norms[own_model_norm.active_])):.2e}]"
    )
    ...
    print(f"Missing indices: {skla - lsa}")
    print(
        f"Coefficient range: [{np.min(abs(own_model_norm2.coef_ / infty_norms[own_model_norm2.active_])):.2e}, {np.max(abs(own_model_norm2.coef_ / infty_norms[own_model_norm2.active_])):.2e}]"
    )
    coefs_norm = np.zeros(d)
